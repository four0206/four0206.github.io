...
function init() {
	...
	$('#form').submit(function (e) {
		// e.preventDefualt is to avoid the form being submitted to the page specified in the action attribute 
   		 e.preventDefault();
   		 // This code is for passing the current form (this) to variable form 
   		 $("#fresults").removeClass('reveal');
   		 var form = this;
   		 // fadeIn is a jQuery function to fadeIn an element 
   		 $(".overlay-container").fadeIn(1000, function(){
   		 		//This code is to call the showFormValues function and pass variable form to it as an argument
   		 		showformValues(form);
...
// This code is to show the form values in the results div with the argument "form"
function showformValues(form){
	//serializeArray is a jQuery function used to get the values of a form as a js object
	var formValues = $(form).serializeArray(); 
	// $.each is a jQQuery alternative to the 'for' loop to iterate through a JS array or object; this is especially beneficial when the length of array is not known
	...
	$.each(formValues, function(index, field){
		...
		// 2) .fund("#"+field.name+"_result") -- finds the element with the id equal to the name of the field being accessed along with text ("_result") Eg : name, pc_result, email_result
